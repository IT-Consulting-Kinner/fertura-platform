...
| E8 | 1 | **(revidiert nach Review)** „Deaktivieren statt löschen" via `active` **plus `deactivated_at`**. Zusätzlich Akteur-Spalten **`created_by`/`updated_by`** (`uuid` NULL, FK→users, ON DELETE SET NULL) auf Fachtabellen, gepflegt vom `FootprintBehavior` aus dem `ActorContext` (HTTP-Identität; CLI/System = NULL). Nicht auf Infra-Tabellen. Ergänzt das Audit-Log (Step 3), ersetzt es nicht. | Kap. 1.6; `deactivated_at` = „wann deaktiviert". Akteur am Datensatz für Herkunft/Verantwortlichkeit. Vom Nutzer bestätigt. |
