...
### 24.13.2 Release-Assembly (Vorstufenschritt vor der Signatur)

Vor der Signatur (24.9) wird ein Modulpaket in einem definierten
Vorstufenschritt zusammengestellt (`bin/cake mp_tool package
<modulverzeichnis> [--previous <x.y.z>]`). Dieser Schritt schließt die Lücke
zwischen „Modul bearbeitet" und „Paket signiert" und erzwingt die
Release-Voraussetzungen, bevor signiert werden darf:

-   **Manifest-/Struktur-Lint** (P16) muss fehlerfrei sein.
-   Die **Version** ist ein gültiges SemVer und gegenüber der Vorversion
    **monoton steigend** (Voraussetzung der Update-Grundregel 28.9.2).
-   **Jede Migration liefert eine umkehrende `@DOWN`-Operation** — die
    Voraussetzung, auf die sich die Rollback-Garantie aus 28.14.2
    („reversible Migrationen") stützt.
-   Für die Version existiert ein **Changelog-Eintrag** (wird im
    Updatefluss angezeigt, 24.13 / 28.9.1).

Ergebnis ist ein bereinigtes, signierfähiges Paketverzeichnis (ohne
Entwicklungs-Artefakte wie Tests, VCS- und Lint-Konfiguration) samt einer
`release.json` (Modulschlüssel, Version, Core-Kompatibilität, Migrationsliste
mit Reversibilitäts-Attribut, Changelog-Text). Da die `release.json` innerhalb
des Pakets liegt, ist sie Teil des signierten Paket-Digests — Changelog und
Reversibilitäts-Attest sind damit manipulationssicher. Erst dieses Verzeichnis
ist Eingabe der Signatur. So gelangen ausschließlich reversible, korrekt
versionierte und dokumentierte Releases in den Signatur- und Auslieferungsfluss.
...
| 6.109 | 12.06.2026 | **Modul-Web-Mount, Stufe 2 — Integration in die Admin-Oberfläche (Kap. 23.16.3)**: Fortsetzung von 6.108. Eine Modul-Seite, die einen Administrationsbereich deklariert, wird jetzt im Admin-Shell des Core dargestellt (Admin-Layout + bereichs-gescopte Seitenleiste); Module tragen ihre Navigationspunkte über einen gemeinsamen Nav-Erbauer bei, den sowohl die Core-Admin-Controller als auch der Modul-Dispatcher nutzen — die Navigation ist dadurch konsistent. Modul-definierte Administrationsbereiche werden bei der Aktivierung registriert (damit sie vergebbar werden) und bei der Deinstallation samt Zuordnungen entfernt. Out-of-Process-isolierte Module dürfen keine Web-Seiten deklarieren (HTML-Rendering ist nicht über die Prozessgrenze möglich); der Manifest-Linter prüft die Web-Routen. Verifiziert: betroffene Tests grün (Admin-Seite im Shell mit Navigationseintrag; Zugriffsverweigerung ohne Bereich), volle Testsuite grün, statische Analyse grün. Weiterhin offen: Gast-/Modul-Authentifizierung der externen API und der lesende Benutzer-/Gruppen-Contract. |
| 6.108 | 12.06.2026 | **Modul-Web-Mount: GUI-tragende Module werden montierbar (Kap. 23.16.3, neue Core-Capability)**: Der Core bot Modulen bisher nur eine JSON-API; es fehlte das Mounting server-gerenderter Modul-Oberflächen (Routen/Controller/Templates/Navigation), wodurch kein GUI-tragendes Modul vollständig betreibbar war. Neu: ein **Core-kontrollierter Web-Dispatcher** unter `/m/<schlüssel>[/<pfad>]` (volle Web-Sicherheitskette: Sitzungs-Authentifizierung, CSRF, Mandanten-RLS, Sicherheits-Header). Der Core behält die Hoheit über Routing, Authentifizierung (Modus pro Seite: angemeldet oder öffentlich/Gast), optionale Admin-Bereichs-Beschränkung, Layout und Ausgabe; ein Modul liefert lediglich eine Handler-Klasse (gibt Anzeigedaten zurück) und eine Template-Datei in seinem eigenen `templates/`-Verzeichnis. Bewusste Entscheidung für einen kontrollierten Dispatcher statt eines generischen Plugin-Ladens (Sicherheits-/Kontroll-Hoheit beim Core). Erste, getestete Ausbaustufe (Engine); offene Folge-Stufen: Manifest-Prüfung der Web-Routen, Navigations-Beitrag der Module in der Admin-Oberfläche, Entwicklerdokumentation sowie die beiden weiteren Capabilities (Gast-/Modul-Token-Authentifizierung der externen API; lesender Benutzer-/Gruppen-Contract). Verifiziert: End-to-End-Test grün (Anzeige, Gastseite, Login-Umleitung, 404), statische Analyse grün, volle Testsuite grün. |
| 6.107 | 12.06.2026 | **Coding-Standard-Altbestand vollständig abgebaut (Reifegrad, keine Spezifikationsänderung)**: Der informative PHPCS-Lauf meldete historisch 1215 Fehler; diese sind nun auf **0** reduziert. Zunächst wurden die maschinell behebbaren Verstöße automatisch korrigiert (Importe, Einrückung, Anführungszeichen, Trailing-Commas u.a.). Der verbliebene Rest wurde zweigleisig erledigt: (a) das Regelwerk wurde **modernisiert** — mit durchgängigen PHP-8.3-Nativtypen und einer PHPStan-Level-8-Prüfung sind verpflichtende Methoden-Docblocks (jede Klasse hat bereits einen Klassen-Docblock), das bewusste „zuweisen-und-prüfen"-Idiom in den API-Controllern, neben dem Test liegende Test-Doubles und die fachlich notwendigen Mehrfachprüfungen kein sinnvolles Signal mehr und wurden entschärft; (b) die **echten Lücken** wurden im Code behoben — 79 fehlende native Rückgabetypen ergänzt, Datenbank-Verbindungs-Hilfsmethoden typgenau auf den konkreten Verbindungstyp gezogen (wodurch sechs echte Rückgabetyp-Abweichungen sichtbar und sauber behoben wurden) sowie kosmetische Whitespace-Reste bereinigt. Die statische-Analyse-Baseline schrumpfte dabei netto um 468 Zeilen ohne eine einzige neue Ausnahme. Es verbleiben 314 nicht-blockierende **Warnungen** (überlange Zeilen, bewusste Fehlerunterdrückung). Verifiziert: volle Testsuite grün, statische Analyse grün. |
| 6.106 | 12.06.2026 | **CI grün gemacht — fehlende Krypto-Umgebungsvariablen (Reifegrad, keine Spezifikationsänderung)**: Der erste CI-Lauf nach dem pgvector-Fix erreichte die Testphase, scheiterte dort aber mit 35 Fehlern und 8 Folgefehlern — alle mit derselben Ursache: der fehlende Verschlüsselungsschlüssel (`Security.encryptionKey`). Die Workflow-Datei setzte die Datenbank- und Redis-Variablen, aber nicht den Anwendungs-Verschlüsselungsschlüssel und das Security-Salt (lokal liefert der Docker-Stack beide). Ohne sie schlagen alle verschlüsselnden Pfade (Zwei-Faktor, SSO, Backup, Settings-Secrets) fehl und sitzungs-/anmeldebehaftete Oberflächen liefern 500. Beide Wegwerf-Schlüssel wurden dem CI-Lauf als Umgebungsvariablen hinzugefügt (ausdrücklich nicht für den Produktivbetrieb). Die übrigen 337 Tests liefen in CI bereits grün; lokal ist die Suite mit denselben Variablen 380 grün. |
| 6.105 | 11.06.2026 | **Englischer Kommentar-Standard, Runde 4 — Testschicht (Wartbarkeit, keine Spezifikationsänderung)**: Die Übersetzung der Code-Kommentare ins Englische (6.99–6.101) auf den gesamten Testbaum (`core/tests`, 87 Dateien über alle Namespaces inkl. Fixtures und Test-Bootstrap) ausgedehnt. Ausschließlich Prosa in Kommentaren/Docblocks; Testmethodennamen, Datenprovider, alle String-Literale — insbesondere die (häufig deutschen) Assertion- und Skip-Meldungen sowie die deutschen Suchbegriffe der Volltext-Tests — blieben unverändert. Verifiziert: Diff nachweislich kommentar-only über alle 87 Dateien, volle Testsuite grün, statische Analyse grün. **Damit ist der komplette Code-Baum (`core/src` und `core/tests`) durchgängig englisch kommentiert.** |
| 6.104 | 11.06.2026 | **CI auf pgvector gefixt + Richtigstellung eines eigenen Doku-Fehlbefunds (Reifegrad, keine Spezifikationsänderung)**: Beim Umsetzen der CI-Folgeaufgabe zeigte sich, dass die in 6.99 als „SQLite-Skeleton" beschriebene Workflow-Datei unter `core/.github/` gar nicht aktiv ist — GitHub liest nur das Wurzel-`.github/`, das bereits eine vollständige PostgreSQL-CI enthält (Postgres-/Redis-Service, PHP 8.3 mit Coverage-Treiber, PostgreSQL-17-Client; Schritte: Tests mit Coverage, Coverage-Ratschet, statische Analyse, Abhängigkeits-Audit, Coding-Standard). Der frühere Befund war also am toten Skeleton gezogen. Eine echte Lücke der aktiven CI wurde behoben: der Datenbank-Service lief auf dem reinen `postgres:17`-Image, dem die von den Migrationen benötigte Vektor-Erweiterung fehlt — auf das pgvector-Image umgestellt (wie in der Entwicklungsumgebung). Die toten Skeleton-Workflows wurden entfernt und der Testleitfaden auf die reale Pipeline richtiggestellt. Verifiziert: Suite lokal gegen pgvector grün, Abhängigkeits-Audit ohne Befund, Workflow-YAML valide. |
| 6.103 | 11.06.2026 | **Abschluss-Audit der Themen-Dokumente (Reifegrad, keine Spezifikationsänderung)**: Die verbleibenden Betriebs-/Entwickler-Leitfäden systematisch (drei read-only Audit-Durchläufe, jeweils mit Code-Beleg) gegen den Ist-Stand geprüft und vier belegte Abweichungen behoben: (a) die Paket-Lizenzangabe in `core/composer.json` stand auf dem Skeleton-Default `MIT` und widersprach der tatsächlichen `LICENSE` (AGPL-3.0) — auf `AGPL-3.0-only` korrigiert; (b) im Modulentwickler-Leitfaden war im Manifest-Beispiel das Feld `collector` statt des vom Linter geforderten `contract` angegeben (ein Modul-Manifest nach altem Stand wäre an der Prüfung gescheitert) — korrigiert; (c) der Anmelde-Bildschirm erhielt den bereits in der Admin-Oberfläche vorhandenen „Zum Inhalt springen"-Sprunglink (Barrierefreiheit, Tastaturnavigation); (d) die Skeleton-Readme des Core-Verzeichnisses wurde durch eine projektgerechte Fassung ersetzt. Die übrigen Leitfäden (Signatur, Lizenzmodell, Mandanten, Mehrsprachigkeit, Runbook, DB-Konventionen, Modul-UI, Programm-Tiers) wurden als akkurat bestätigt; bewusst unverändert blieb der als Konzept gekennzeichnete Abschnitt zu Integrations-Extension-Modulen. |
| 6.102 | 11.06.2026 | **Doku-Code-Abgleich + Schließen einer Token-Scope-Lücke (Reifegrad, keine Spezifikationsänderung)**: Die Themen-Leitfäden gegen den realen Code geradegezogen — `API.md` (durchgesetzte Scopes `audit:read`/`scim:manage` und die neuen Endpunkte Audit-Export/Suche/Benachrichtigungen/OpenAPI/SCIM ergänzt; veralteter „kein Rate-Limiting"-Hinweis korrigiert, da `ApiRateLimitMiddleware` aktiv ist), `TESTING.md` (Coverage ist jetzt ein anhebender Ratschet-Gate; Mutation-Testing/Infection ergänzt; Sicherheits-Integrationstests aufgenommen; der CI-Abschnitt ehrlich auf den Skeleton-Stand gezogen — der Test-Job läuft noch gegen SQLite und deckt die PostgreSQL-Integrationssuite nicht ab, maßgeblich ist der Docker-Lauf), `BACKUP.md` (die Core-Off-Site-Ablage ins Objekt-Storage dokumentiert, vorher nur als Infra-Aufgabe vermerkt), `README.md` (Status realistisch gefasst, Doku-Index vervollständigt). Dabei eine **echte Bedienlücke** behoben: die von SCIM-Provisioning und Audit-Export durchgesetzten Token-Scopes waren in der Admin-GUI nicht als Least-Privilege-Token vergebbar (nur über die `*`-Wildcard) — `TokenService::KNOWN_SCOPES` entsprechend ergänzt (Token-Tests grün). Eine Folgeaufgabe wurde vermerkt: die CI von SQLite auf einen PostgreSQL-17-Dienst umstellen, damit die Integrationssuite auch in CI greift. |
| 6.101 | 11.06.2026 | **Englischer Kommentar-Standard, Runde 3 — restlicher Code-Baum abgeschlossen (Wartbarkeit, keine Spezifikationsänderung)**: Abschluss von 6.99/6.100. Alle übrigen 104 Dateien außerhalb der Service-Schicht auf englische Kommentare/PHPDoc übersetzt — Controller (Login/MFA/SSO/Health/SSE/Metrics, Admin-Bereiche, API inkl. SCIM und v1), sämtliche CLI-Kommandos, Middleware (Authentifizierung/Rate-Limit/Security-Header/Session-Wächter/RLS), Datenmodell (Entities/Tables/Behaviors), Event-/Log-/Auth-/Infrastruktur-/Lokalisierungs-/View-Schicht, Audit-Logger und Bootstrap. Ausschließlich Prosa; Code, Typ-Annotationen, benutzersichtbare CLI-Hilfe-/Flash-/Übersetzungs-/Fehlermeldungs-Texte und alle Referenzen blieben unverändert; deutsche Beispielpfade in Hilfe-Docblocks wurden zu neutralen Platzhaltern verallgemeinert. Verifiziert: Umlaut- und Deutsch-Wort-Scan über Kommentare = 0, Diff nachweislich kommentar-only über alle 104 Dateien, statische Analyse grün, volle Testsuite grün. **Damit ist der gesamte produktive Code-Baum (`core/src`) durchgängig englisch kommentiert** (rund 209 Dateien über drei Runden). |
| 6.100 | 11.06.2026 | **Englischer Kommentar-Standard, Runde 2 — restliche Service-Schicht (Wartbarkeit, keine Spezifikationsänderung)**: Fortsetzung von 6.99. Alle verbliebenen 96 Dateien der Service-Schicht (KI-Gateway, API/Token, Audit-Export, lokale Auth + SSO, Automation/Workflow, Backup/Off-Site, Cache, Migrations-Sicherheit, Outbox, Export, Health, Egress/SSRF-Schutz, Lokalisierung, Passwort-Reset, Lizenz, Mail, Marketplace, Metriken, Modul-Lebenszyklus/Isolation, Benachrichtigung, Observability, Berechtigungen/RLS, Anonymisierung, Queue, Realtime, Contract-Registry, Scheduler, SDK, Suche, Einstellungen/Secrets, Storage, System-Flags, Mandanten, Update, Webhooks) auf englische Kommentare/PHPDoc übersetzt. Ausschließlich Prosa; Code, Typ-Annotationen, alle Kapitel-/Entscheidungs-/RFC-Referenzen und die (deutschsprachigen) Laufzeit-Fehlermeldungen blieben unverändert. Verifiziert: Rest-Deutsch-Scan = 0, Diff nachweislich kommentar-only, statische Analyse grün, volle Testsuite grün. |
| 6.99 | 11.06.2026 | **Englischer Kommentar-Standard, Runde 1 (Wartbarkeit, keine Spezifikationsänderung)**: Code-Kommentare/PHPDoc werden going-forward auf klares, modernes Englisch geführt (internationale und KI-Tooling-Zugänglichkeit; der Code selbst ist ohnehin englisch). Bewusst inkrementell und scope-begrenzt — **Service-Schicht zuerst**, danach Controller/Command; Templates, Migrationen und Config bleiben ausgenommen (Boilerplate/Daten, geringer Wartungs-Grenznutzen). Erste Runde: die neun sicherheitskritischsten Dateien (`Service/Security`: CBOR-Codec, MFA/TOTP, Paketsignatur-Prüfung, Signierer, Vertrauenskette/-speicher, WebAuthn). Übersetzt wurde ausschließlich Prosa; Code, Typ-Annotationen und alle Kapitel-/Entscheidungs-/RFC-Referenzen blieben unverändert, ebenso die (deutschsprachigen) Laufzeit-Fehlermeldungen. Verifiziert: Diff ist nachweislich kommentar-only, statische Analyse grün, Sicherheits-Testsuite grün. |
| 6.97 | 11.06.2026 | **Mutation-Testing für die Sicherheits-Kernmodule (Test-Qualität; keine Spezifikationsänderung)**: Mutation-Testing (Infection) eingeführt — eingebaute, gezielte Code-Veränderungen prüfen, ob die bestehenden Tests eine Regression tatsächlich erkennen (aussagekräftiger als reine Zeilenabdeckung). Bewusst eng begrenzt auf die sicherheitskritischsten Bereiche: Signatur-/Vertrauenskette und Zwei-Faktor-Verfahren, Lizenz-Statusmaschine, Berechtigungslogik (deny-wins) und Authentifizierung. Als optionales Qualitätswerkzeug (nicht im blockierenden CI-Lauf, da rechenintensiv); liefert eine Baseline-Kennzahl, deren überlebende Mutanten künftige Test-Härtung zielgenau leiten. |
| 6.96 | 11.06.2026 | **Gezielte Testabdeckung sicherheitsnaher Oberflächen (Reifegrad, keine Spezifikationsänderung)**: Datengestützte Auswahl statt Zahlen-Jagd — die zwei verbliebenen, gut testbaren **Sicherheits-Glue**-Bereiche abgedeckt: die **MFA-Selbstverwaltung** (TOTP-Einrichtung mit Bestätigung und Recovery-Codes, Deaktivierung, Passkey-Registrierung/-Löschung inkl. Options-Gate) und der **Einladungs-/Passwort-Reset-Token-Lebenszyklus** (Hash-only-Speicherung, Einlösung mit Aktivierung, Ablehnung zu kurzer/ungültiger/abgelaufener/verbrauchter Token). Bewusst aufgeschoben bleiben SSO-Flows (benötigen IdP-Stubs; die Kryptografie ist bereits getestet), Daemon-/Streaming-Schleifen und modulabhängiger Code (deckt sich mit dem ersten Modul ab). Verifiziert: 13 neue Tests; volle Suite **380 grün**, PHPStan grün, Coverage-Gate grün. |
| 6.95 | 11.06.2026 | **CLI↔GUI-Paritäts-Audit + drei ergänzte Verwaltungs-GUIs (Kap. 20.1/24.9.2)**: Vollständiger Abgleich aller CLI-Kommandos gegen die Admin-Oberfläche nach dem Paritäts-Prinzip (6.94). Drei verbliebene Lücken in der GUI geschlossen: **(1) Suchindex neu aufbauen** (Volltext + fehlende Embeddings nachziehen), **(2) Off-Site-Sicherung** (lokales Backup ins Objekt-Storage laden, Off-Site-Dateien auflisten/löschen — nur sichtbar, wenn aktiviert), **(3) Vertrauensanker & Sperrliste** als eigener Bereich (Anzeige inkl. Gültigkeit und Sperrlisten-Alter, Schlüssel widerrufen mit Markierung betroffener Module, Anker manuell hinzufügen mit harter Bestätigung). Korrekt CLI-only bleiben weiterhin Bootstrap-/Infrastruktur-, destruktive (Wiederherstellung), Entwickler-/SDK-, Diagnose- und Marketplace-Betreiber-Kommandos. Verifiziert: GUI-Tests je Funktion (Berechtigung, Widerruf, Anlage, Gating); volle Suite **367 grün**, PHPStan grün, Coverage-Gate grün. |
| 6.94 | 11.06.2026 | **Volle GUI-Parität für Integrationen + CLI↔GUI-Prinzip (Kap. 26)**: Auch **Automations-Regeln** und **Workflows** lassen sich jetzt in der GUI anlegen (zuvor nur per CLI) — mit derselben validierten JSON-Eingabe für Bedingungen/Aktionen/Übergänge, die die CLI verwendet. Damit gilt für die GUI dieselbe Funktionsbreite wie für die CLI. Festgehalten als **verbindliches Prinzip**: Jede Core-CLI-Funktion ist auch über die GUI konfigurierbar, sofern keine harten Sicherheitsgründe dagegensprechen (Ausnahme bleibt die destruktive Daten-Wiederherstellung = CLI-only). Ein komfortabler visueller Regel-Builder ist eine spätere, ebenfalls Core-seitige Komfortstufe und keine Voraussetzung der Parität. Verifiziert: erweiterte GUI-Tests (Anlage + JSON-Validierung je Typ); volle Suite **362 grün**, PHPStan grün, Coverage-Gate grün (60,5 %). |
| 6.93 | 11.06.2026 | **Anlege-GUI für Integrationen (Admin-GUI-Restpunkt; Kap. 26/27/29)**: Die Verwaltungsfläche „Integrationen & Automatisierung" erlaubt jetzt das **Anlegen** der formular­geeigneten Typen direkt in der GUI statt nur per CLI: **Webhook-Abos** (Name/URL/Event-Filter/HMAC-Secret, mit URL-Validierung) und **SSO-Provider** (OIDC und SAML; Client-Secret verschlüsselt gespeichert) — letzteres schließt die zuvor als „spätere Ergänzung" notierte SSO-Admin-GUI. Automations-Regeln und Workflows bleiben bewusst CLI-seitig (regel-/JSON-lastig; ein visueller Regel-Builder gehört an das künftige deklarative Formular-Schema). Verifiziert: erweiterte GUI-Tests (Anlage, Validierung, Secret-Verschlüsselung); volle Suite **360 grün**, PHPStan grün, Coverage-Gate grün (60,4 %). |
| 6.92 | 11.06.2026 | **Audit-Export & SIEM-Anbindung (Kap. 20.2/24.16 erweitert; Compliance-Reife)**: Das unveränderliche Audit-Log wird enterprise-tauglich ausgeleitet — **(a)** jedes Ereignis erscheint zusätzlich auf einem **dedizierten Audit-Log-Kanal** (eigene Datei, strukturiertes JSON), den der Betreiber per Log-Shipper an ein beliebiges SIEM (Splunk/Sentinel/Elastic …) weiterleitet; bewusst ohne vendorspezifische Konnektoren im Core und **datensparsam** (Personen per UUID, keine Wert-Snapshots im Strom). **(b)** Ein **gezielter Export** über Zeitbereich/Filter als NDJSON-Strom — als Administrator-Download (mit Wert-Snapshots) und über die API (Token-Scope `audit:read`, Snapshots zuschaltbar). Speicherschonend per Keyset-Streaming. Damit ist die häufigste Enterprise-Beschaffungsanforderung (SIEM-Integration + Auditor-Pull) erfüllt, die die meisten vergleichbaren OSS-Plattformen nicht ab Werk bieten. Verifiziert: Service- und Endpunkt-Tests (Filter, Scope-Gate, Datensparsamkeit); volle Suite **357 grün**, PHPStan grün, Coverage-Gate grün (60,3 %). |
| 6.91 | 11.06.2026 | **Passkeys, Session-Anomalie-Erkennung und SCIM-Provisioning (Sicherheits-Kür; Kap. 27.16.3/29 erweitert)**: Drei weitere, marktführende Sicherheits-/Enterprise-Funktionen — alle ohne neue Abhängigkeit. **(1) Passkeys/WebAuthn** als zweiter Faktor (Fingerabdruck/Gesicht/Security-Key): Standard-konforme Registrierung und Anmeldung mit voller serverseitiger Prüfung (Herkunft, Signatur, Klon-Erkennung über den Sign-Zähler), als bequeme Alternative zum Code; Wiederherstellung weiter über die Recovery-Codes. **(2) Session-Anomalie-Erkennung**: Bindung der Sitzung an den Browser (gestohlenes Cookie in anderem Browser beendet die Sitzung), Protokollierung von IP-Wechseln (strenger Modus optional) und ein In-App-Hinweis bei Anmeldung von einem **neuen Gerät**. **(3) SCIM 2.0** (`/api/scim/v2`, Token-Scope `scim:manage`): Benutzer-Provisioning durch Identitäts-Provider (Entra ID/Okta) — Anlage als „eingeladen", Filter/Paging, Aktualisierung und Deaktivierung; Löschen deaktiviert (kein hartes Löschen — Anonymisierung bleibt ein bewusster DSGVO-Schritt). Verifiziert: WebAuthn-Roundtrip mit echtem Schlüsselpaar inkl. aller Fail-closed-Pfade, Passkey-Login-E2E, Session-Guard- und SCIM-Tests; volle Suite **351 grün**, PHPStan grün (Baseline unverändert), Coverage-Gate grün (60,0 %). |
| 6.90 | 11.06.2026 | **Zwei-Faktor-Authentifizierung, Security-Header und Abhängigkeits-Scanning (Kap. 27.16.3 erweitert)**: Die drei in der State-of-the-art-Bewertung benannten Sicherheitslücken wurden geschlossen. **(1) MFA (TOTP)** für lokale Konten: Standard-konforme Einmal-Codes (RFC 6238, kompatibel mit allen gängigen Authenticator-Apps, bewusst ohne neue Abhängigkeit), zweistufige Self-Service-Einrichtung (Secret erst nach Code-Bestätigung verschlüsselt gespeichert), **Einmal-Recovery-Codes** (nur als Hash gespeichert), Anmeldung zweistufig (Passwort → Code; Identität wird erst nach gültigem zweiten Faktor gesetzt), Wiederholungs-Schutz für Codes, Fehlversuche in der bestehenden Anmelde-Drosselung; optional per Betreiber-Setting **erzwingbar**; SSO-Anmeldungen ausgenommen (MFA-Policy liegt beim Identitäts-Provider). **(2) Security-Antwort-Header** auf jeder Antwort inkl. Fehlerseiten: Content-Security-Policy (self-hosted, kein Framing), X-Frame-Options, nosniff, Referrer-/Permissions-Policy, HSTS nur über TLS; per Settings an Betreiber-Proxys anpassbar. **(3) SCA**: bekannte Sicherheitslücken in Produktiv-Abhängigkeiten brechen die CI (`composer audit`), wöchentliche Update-PRs (Dependabot). Verifiziert: RFC-Testvektoren, Service-/E2E-Login-Tests, Header-Tests inkl. 404, Runtime-Smoke; volle Suite **336 grün**, PHPStan grün (Baseline geschrumpft), Coverage-Gate grün (59,1 %). |
| 6.89 | 11.06.2026 | **Systematische Härtung gegen fehlgeformte IDs in URLs (Robustheit, keine Spezifikationsänderung)**: Das in 6.88 punktuell behobene Muster — eine fehlgeformte ID in einer Admin-/API-URL führte über den ungeprüften Datenbank-uuid-Vergleich zu einem **Serverfehler statt „nicht gefunden"** — wurde codebasis-weit geschlossen: ein zentraler Wohlgeformtheits-Prüfer plus Guards an allen betroffenen Stellen (Token-Widerruf, Dead-Letter-Retry/-Verwerfen, SSO-/Webhook-/Automations-/Workflow-Verwaltung, Benachrichtigungen-API, Mandanten- und Benutzer-/Gruppenverwaltung, Backup). Fehlgeformte Werte werden überall wie unbekannte IDs behandelt (Hinweis + Umleitung bzw. No-op); ORM-Pfade waren nicht betroffen. Ausgeführt als separater Hintergrund-Task, anschließend reviewt. Verifiziert: zentraler Guard-Test + Malformed-ID-Regressionstests in 6 Test-Suiten; volle Suite **320 grün**, PHPStan grün, Coverage-Gate grün (58,6 %). |
| 6.88 | 11.06.2026 | **Testabdeckung Marketplace-Kanal + Admin-Screens, behobener Robustheits-Fehler (Kap. 28.4–28.6/24.9.2; keine Spezifikationsänderung)**: Die **Marketplace-Kommunikation** wurde gegen einen HTTP-Stub mit echter Signaturkette getestet: Nur signiert verifizierte Sperrlisten-/Anker-Dokumente werden angewendet (manipulierte verworfen, Entscheidung 139), Publisher-Anker nur mit gültiger Root-Signatur (Kette), der Sperrlisten-Abruf wird datiert und Veralterung erkannt (Stale-Warnung). Ergänzend ein Render-Smoke aller verbliebenen Admin-Screens (Dashboard, Audit, Backup, Updates, Registry, Marketplace) inkl. Bereichs-Pflicht. **Dabei gefunden und behoben:** eine fehlgeformte Backup-ID in der URL führte zu einem **Serverfehler statt „Backup unbekannt"** (ungeprüfter uuid-Vergleich); an der Service-Grenze abgesichert, gleiches Muster in übrigen Admin-Routen als Folge-Aufgabe markiert. Mindest-Abdeckung in CI erneut angehoben (55,8 → 58,1 %, Floor 57,7). Verifiziert: 8 neue Tests; volle Suite **312 grün**, PHPStan grün, Coverage-Gate grün. |
| 6.87 | 11.06.2026 | **Testabdeckung Lizenzierung + Modul-Lifecycle-GUI (Kap. 28.7/23.13.1; Reifegrad, keine Spezifikationsänderung)**: Die Offline-first-**Lizenz-Statusmaschine** wurde vollständig gegen die echte Ed25519-Vertrauenskette getestet — Datei-Validierung (manipulierter Inhalt, unbekannter/abgelaufener/widerrufener Signaturschlüssel werden abgelehnt), Installation als Upsert, und die komplette Status-Matrix: gültig / **Karenzfenster** (abgelaufen, aber Aktivierung bleibt erlaubt) / abgelaufen / widerrufen / fehlend, inkl. **Online-Enforcement** (überfällige Server-Bestätigung → Nachprüfung erforderlich; frische Bestätigung → gültig; Überfälligkeit im Karenzfenster → Karenz). Ebenso die **Modul-Lifecycle-GUI**: Liste, geschichtetes Abhängigkeits-Diagramm (SVG) und die Fehlerpfade der Aktionen (sauberer Hinweis statt Serverfehler). Kein neuer Fehler; die Entscheidung-158-Regeln verhalten sich wie spezifiziert. Mindest-Abdeckung in CI erneut angehoben (53,8 → 55,8 %, Floor 55,4). Verifiziert: 8 neue Tests; volle Suite **304 grün**, PHPStan grün, Coverage-Gate grün. |
| 6.86 | 11.06.2026 | **Testabdeckung Sprachverwaltung (Kap. 31; Reifegrad, keine Spezifikationsänderung)**: Das komplette Sprachverwaltungs-Subsystem — die größte bislang ungetestete Fläche — erhielt Service- und GUI-Integrationstests: Import (Validierung ungültiger PO-Dateien, Vorschau mit Überschreib-Warnung bei editierten Zielen, Übernahme als unsigniertes Upload-Pack), **verlustfreies Editieren** (nur der geänderte Eintrag ändert sich; Admin-Bearbeitung gilt als Review), Review-Markierung, die **Lösch-Regeln** (Englisch ist bei aktiver Komponente geschützt, bei entfernter Komponente löschbar) und der **Versions-Status** der Übersicht (aktuell / gleiche Hauptversion / inkompatibel). Kein neuer Fehler gefunden; die E38/E41/E42-Regeln verhalten sich wie spezifiziert und sind jetzt regressionsgesichert. Mindest-Abdeckung in CI erneut angehoben (51,5 → 53,8 %, Floor 53,5). Verifiziert: 15 neue Tests; volle Suite **296 grün**, PHPStan grün, Coverage-Gate grün. |
| 6.85 | 11.06.2026 | **Testabdeckung Benutzerverwaltung + Dead-Letter-Verwaltung (Reifegrad, keine Spezifikationsänderung)**: Die bislang ungetestete **Benutzer-Admin-GUI** (Kap. 27.14/27.15) erhielt einen vollständigen Integrationstest — insbesondere alle **Selbst-Aussperr-Schutzmechanismen** (kein Selbst-Deaktivieren/-Anonymisieren, letzter aktiver Benutzerverwaltungs-Administrator geschützt, Aktivierung nur mit gesetztem Passwort), der **Einladungsfluss** (Token-Erzeugung) und das Admin-Passwort-Setzen (Mindestlänge, Aktivierung eingeladener Benutzer). Ebenso die **Dead-Letter-Verwaltung** des Event-Outbox (Kap. 26.9.2): Wiedereinstellen setzt Zähler/Sperre/Fehler zurück und wirkt **nur** auf fehlgeschlagene Events, Verwerfen ist terminal, Sammel-Wiedereinstellung lässt verarbeitete Events unangetastet. Kein neuer Fehler gefunden — die Schutzmechanismen verhalten sich wie spezifiziert und sind jetzt regressionsgesichert. Mindest-Abdeckung in CI erneut angehoben (49,0 → 51,5 %, Floor 51,2). Verifiziert: 16 neue Tests; volle Suite **281 grün**, PHPStan grün, Coverage-Gate grün. |
| 6.84 | 11.06.2026 | **Testabdeckung der Gruppenverwaltung + behobener Produktions-Bug (Korrektheits-Härtung, keine Spezifikationsänderung)**: Die bislang ungetestete RBAC-Gruppenverwaltung (Liste/Anlegen/Detail/Aktiv-Schalter/Mitgliedschaft/Rechtevergabe) erhielt einen vollständigen Integrationstest. **Dabei aufgedeckt und behoben:** das **Deaktivieren einer Gruppe** schlug fehl — ein boolescher SQL-Parameter (`false`) wurde als leerer Text gebunden, den PostgreSQL für ein Boolean-Feld ablehnt; nur der Ausschalt-Pfad war betroffen (gleiche Fehlklasse wie zuvor bei den Benachrichtigungen). Ein systematischer Folge-Scan über alle direkten SQL-Aufrufe bestätigte: dies war die **letzte** verbliebene Stelle, alle übrigen nutzen bereits die sichere Bindung. Die in CI verankerte Mindest-Abdeckung wurde entsprechend angehoben. Verifiziert: neuer Controller-Test; volle Suite **265 grün**, PHPStan grün, Coverage-Gate grün (49,0 %). |
| 6.83 | 10.06.2026 | **Peer-Review der neuen Skalierungs-/Reife-Pfade + Remediation (Korrektheits-/Sicherheits-Härtung, keine Spezifikationsänderung)**: Ein 4er-Panel (vier unabhängige adversariale Prüfungen) deckte mehrere Schwächen in der jüngsten Arbeit auf; die bestätigten wurden behoben. **(Sicherheit)** Bei DB-pro-Mandant konnten zwei isolierte Mandanten, deren Schlüssel sich nur durch `-` vs. `_` unterschieden, auf dieselbe Verbindungs-Env und damit **dieselbe Datenbank** abgebildet werden (Cross-Tenant-Leck) — jetzt **fail-closed** abgewiesen. **(Korrektheit)** Die Mandanten-eigene Verbindung erhielt nicht das `core`-Schema-Profil der Hauptverbindung (unqualifizierte Abfragen/Migrationen hätten ins falsche Schema gegriffen) — jetzt identisches Profil. **(Durchsatz)** Der faire Outbox-Claim begrenzte VOR dem Sperren, sodass mehrere Worker um dieselben Datensätze konkurrierten und fast leere Stapel zogen — jetzt wird nach dem Sperren begrenzt (echter Mehr-Worker-Durchsatz). **(Korrektheit)** Der Health-Alert-Zustand lag in einem Cache mit 1-Stunden-Ablauf, sodass eine länger anhaltende Störung stündlich denselben Alarm erneut auslöste — jetzt langlebiger Zustand. **(Robustheit)** Redis-Reclaim paginiert vollständig; fehlgeschlagene Telemetrie-/Alert-Zustellungen (z. B. interner Collector hinter dem SSRF-Schutz) werden jetzt **geloggt** statt still verworfen (Allowlist-Hinweis dokumentiert); Coverage-Gate weist strukturell leere Berichte hart ab. Verifiziert: neue Mandanten-Tests; volle Suite **256 grün**, PHPStan grün, Coverage-Gate grün. |
| 6.82 | 10.06.2026 | **Reife- & Prozess-Härtung (Konvention, Barrierefreiheit, Qualitäts-Gates)**: **(1)** **Zentral/Tenant-Daten-Konvention festigen** — neue Fassade `Tenancy` macht die Trennung explizit: `central()` (geteilte DB für mandanten­übergreifende Tabellen — Benutzer/Mandanten/Sessions/Settings/Audit/Events …) vs. `data()` (bei DB-pro-Mandant die eigene DB, sonst Pool). Bewusst **kein** pauschales Re-Aliasing der Default-Verbindung (Auth-/Session-Bootstrap muss zentral aufgelöst sein, Henne-Ei); `TENANCY.md` dokumentiert die Regel. **(2)** **Barrierefreiheit systematisch** — alle handgebauten Core-Admin-Tabellen (18) tragen jetzt `scope="col"` (zusätzlich zu den UiKit-generierten); neue **`A11Y.md`-Checkliste** als verbindliches Minimum pro Screen (Core & Modul) inkl. Auflistung der vom Core schon gelösten Belange (UiKit, FormHelper-Fehler-ARIA, Skip-Link, Landmarks, Live-Region, Fokus). Befund: FormHelper emittiert Fehler-ARIA automatisch; kein Icon-Font → keine namenslosen Buttons. **(3)** **Qualitäts-Gates geschärft** — blockierendes **Coverage-Ratschet-Gate** in CI (`coverage-min.txt`, Start 47 %, nur anhebbar) und gezielter **PHPStan-Baseline-Abbau** (4 Befunde sauber behoben statt grandfathered, Diff = nur Entfernungen); dabei **8 latente, nicht-baseline-te PHPStan-Fehler aus der Observability-Einführung behoben** (CI war rot) → `phpstan analyse` jetzt fehlerfrei. Verifiziert: volle Suite **254 grün**, PHPStan grün, Coverage-Gate grün. |
| 6.81 | 10.06.2026 | **Observability als Core-Emission (Grenzfall #12)**: Schließt den letzten Grenzfall des Reife-Reviews. **(a)** **Schlanker OTLP/HTTP-Metrik-Export** — die vorhandenen Prometheus-Gauges werden zusätzlich im **OpenTelemetry-Format** an `OTEL_EXPORTER_OTLP_ENDPOINT` exportiert (über den gehärteten Egress, ohne schweres SDK); jeder OTel-fähige Stack (Collector/Grafana/Datadog …) ingestiert, kein Lock-in. **(b)** **Signierter Health-Alert-Hook** — meldet **Statuswechsel** des Gesamt-Health (`up`↔`degraded`/`down`) per HMAC-signiertem Webhook an `health.alert_url`; nur bei Übergang (kein Spam), Routing/Eskalation übernimmt der externe Empfänger. Beide laufen gedrosselt und fehlerisoliert im Worker-Zyklus, nur bei Konfiguration aktiv. Bewusst **dünner Exporter statt OTel-PHP-SDK** (Emission statt Backend); Traces bleiben über `traceparent`/Logs korrelierbar. Verifiziert per Test (OTLP-Payload-Struktur + Versand; Alarm nur bei Übergang, signiert). |
| 6.80 | 10.06.2026 | **Skalierung & Multi-Tenant-Betrieb (Thema #10, alle 5 Punkte)**: Konsequenz aus der Mandantenfähigkeit. **(1)** **Faire Outbox** — der Worker arbeitet Events mandanten-fair (Round-Robin) ab, kein Mandant monopolisiert; Pro-Mandant-Cap je Batch. **(2)** **Queue-Transport** — neues durables Job-Queue-Primitiv mit zwei Treibern (Postgres-Default **und** Redis Streams als Broker-Pfad), getrennt vom bewährten Event-Outbox. **(3)** **DB-Sessions als Default** — Web-Schicht instanzübergreifend/HA-fähig. **(4)** **DB-pro-Mandant-Option** — ein Mandant kann auf einer eigenen Datenbank laufen (stärkste Isolation); Verbindungs-Zugangsdaten out-of-band, fail-closed bei Fehlkonfiguration; Provisionierungs-Befehl. **(5)** **`SCALING.md`** — dokumentiert die Grenze Core (Mechanismus/Emission) ↔ Betrieb (Load-Balancer, Skalierung, Broker-/Replica-Betrieb, WAL-PITR, Observability-Backend) + Referenz-Topologie. Single-Org/On-Prem bleibt unverändert lauffähig. Verifiziert: neue Tests je Punkt; volle Suite **239→248 grün**; PHPStan grün. |
| 6.79 | 10.06.2026 | **Core-seitige Reife-Themen (C): Such-Tiefe, RBAC-Deny, Barrierefreiheit**: **(1) Sprachbewusste Suche** — der Volltextindex kombiniert exakte (`simple`) mit sprachlicher Analyse (Stemming/Stopwords, Default Deutsch, konfigurierbar); Beugungen werden gefunden, exakte Treffer bleiben erhalten. **(2) RBAC-Deny-Regeln** — die Rechtevergabe kennt jetzt **explizite Verbote** (deny-wins): ein Verbot auf einer Gruppe überschreibt jede Erlaubnis, auch gruppenübergreifend; Grundlage für feinere Enterprise-Autorisierung. **(3) Barrierefreiheit** — erste WCAG-Härtung der Admin-/Login-Oberfläche und des UI-Kits: Sprung-zum-Inhalt-Link, benannte Landmarks, `aria-current`/`aria-sort`, Live-Region für Meldungen, sichtbarer Fokus, `scope`-Spaltenköpfe. (Ausgeschlossen, da Modul/Produkt: echte Business-Module, visueller Builder.) Verifiziert: neue Tests je Thema; volle Suite **235→239 grün**; PHPStan grün. |
| 6.78 | 10.06.2026 | **Core-seitige Multi-Tenant-Restpunkte + Schema-Cache-Fix**: **(1) Pro-Mandant-Settings** — ein Setting kann global **oder je Mandant** überschrieben werden (mandantenspezifisch > global > Katalog-Default); ermöglicht z. B. Limits/Feature-Flags je Mandant. **(2) Mandanten-Lifecycle** — ein Mandant lässt sich (samt seiner Such-/Embedding-Daten) löschen; Default-Mandant und Mandanten mit Benutzern sind geschützt; GUI-Sammelaktion mit Bestätigung. **(3) Behoben:** ein latenter Fehler, bei dem nach einer Schema-Migration neue Spalten von der ORM-Schicht stillschweigend ignoriert wurden (Metadaten-Cache wird jetzt nach Migrationen geleert). Bewusst ausgeschlossen (Modul/Extension): Self-Service-Mandantenanlage, Abrechnung, Tenant-Scoping der Modul-Tabellen. Verifiziert: neue Tests; volle Suite **233→235 grün**; PHPStan grün. |
| 6.77 | 10.06.2026 | **Multi-Tenant-Tiefe (entschiedene B-Restpunkte)**: Gemäß den getroffenen Entscheidungen umgesetzt — **(1)** **Such-/Embedding-Index mandantenscharf**: `tenant_id` + pro-Mandant-Eindeutigkeit + mandantengefilterte Abfragen schließen das mandantenübergreifende Leck öffentlicher Dokumente. **(2)** **Mandant am Event**: jedes Outbox-Event trägt seinen Mandanten; der Worker verarbeitet im richtigen Mandantenkontext (korrekte Indizierung etc.). **(3)** **Cross-Tenant-Host-Policy**: ein angemeldeter Benutzer auf der Domain eines fremden Mandanten wird abgewiesen (abschaltbar). **(4)** **Mandanten-Branding**: Anzeigename/Logo je Mandant auf der host-aufgelösten Login-Seite, im Mandanten-Formular pflegbar. Inline-Edit bewusst nicht umgesetzt (SSR-aktionsbasiert, Nutzerentscheidung). Verbleibend: Adoption des Tenant-Musters in den Modul-Tabellen. Verifiziert: neue Tests je Punkt; volle Suite **228→233 grün**; PHPStan grün. |
| 6.76 | 10.06.2026 | **Anschluss-Ausbau der Wettbewerbs-Hebel (offene B-Punkte)**: **(1) Suche:** Embedding-**Backfill** für Bestandsdaten (`bin/cake search_reindex --embeddings`) und eine **globale Admin-Suche** (`/admin/search`, Hybrid, mit UI-Kit). **(2) UI-Kit:** **Sammelaktionen** (Auswahl-Spalte + Aktions-Toolbar, SSR-nativ) — erstmals genutzt für das Aktivieren/Suspendieren von Mandanten. **(3) Mandantenfähigkeit:** **Pre-Auth-Auflösung** des Mandanten aus dem Request-Host/der Subdomain (mandantenspezifische Login-Oberfläche), **Propagation** des Mandanten an isolierte Modul-Hosts (RPC), sowie **GUI**-Funktionen zum Suspendieren und zur Benutzer-↔-Mandant-Zuweisung. Verbleibend (dokumentiert): Worker-/Event-Propagation, Tenant-Scoping echter Datentabellen, mandantenspezifisches Branding. Verifiziert: neue Tests je Punkt; volle Suite **221→228 grün**; PHPStan mit Baseline grün. |
| 6.75 | 10.06.2026 | **Folge-Ausbau der Wettbewerbs-Hebel + CI-Härtung**: **(1)** **Hybrid-Suche auto-indexiert** — beim Indexieren eines Suchdokuments wird (per Schalter `ai.embed.auto_index`) automatisch auch das Embedding erzeugt; kein Modul muss mehr doppelt schreiben. **(2)** **UI-Kit praxistauglich** — Sortierung, Paginierung und Filter ergänzt und erstmals in einer echten **Mandanten-Verwaltungs-GUI** (`/admin/tenants`) eingesetzt (deckt zugleich die offene Multi-Tenancy-Admin-GUI ab). **(3)** **CI gehärtet** — statische Analyse (PHPStan Level 8) ist jetzt ein **blockierendes Gate** (bestehende Befunde via Baseline grandfathered, neue brechen die CI), Coverage-Messung (pcov) im Lauf, PHPCS informativ; zudem fehlende `gd`-PHP-Erweiterung in der CI ergänzt (verhinderte sonst denselben Plattform-Konflikt wie beim Worker). Verifiziert: neue Tests je Punkt; volle Suite **216→221 grün**; PHPStan mit Baseline grün. |
| 6.74 | 10.06.2026 | **Drei Wettbewerbs-Hebel umgesetzt** (Antwort auf die Marktanalyse 6.73): **(1) Hybrid-Suche** — Volltext (`tsvector`) und semantische pgvector-Ähnlichkeit werden über Reciprocal Rank Fusion vereint (`SearchService::hybrid`, API `mode=hybrid`); ohne Embedding-Provider sauberer Rückfall auf Volltext. Verbindet die zuvor getrennten Such-Hälften. **(2) Modul-UI-Kit** — `UiKitHelper` (`$this->UiKit`) rendert deklarative, HTML-sichere CRUD-Bausteine (Liste/Detail/Formular) im einheitlichen Stil; Module brauchen kein handgeschriebenes Markup mehr (`MODULE_UI.md`). **(3) Mandantenfähigkeits-Fundament** — Mandanten-Tabelle + Default-Mandant, `users.tenant_id`, Mandantenkontext pro Request über RLS (`core.current_tenant()`), `TenantService`; **fail-closed** (ohne Kontext keine mandanten-bezogenen Daten → keine Teil-Einführungs-Lecks), Single-Org unverändert (`TENANCY.md`). Nächste SaaS-Schritte (Pre-Auth-Host-Auflösung, RPC-Propagation, Admin-GUI) dokumentiert. Verifiziert: neue Tests je Hebel; volle Suite **206→216 grün** + Runtime-Smoke. |
| 6.73 | 10.06.2026 | **Drittes Peer-Review (Regression + Breitensweep + Reife-/Marktanalyse) + Remediation**: Bestätigte Befunde behoben — **(1)** Index für die Per-IP-Anmeldedrosselung (`auth_failures(ip_address, occurred_at)`): ohne ihn hätte jeder Login-POST einen Tabellen-Scan ausgelöst (Regression aus 6.72). **(2)** Egress löst den Ziel-Host pro Anfrage nur noch **einmal** auf (vorher bis zu vierfach) — Policy-Prüfung und IP-Pinning zusammengeführt. **(3)** Session- und CSRF-Cookies explizit `HttpOnly` + `SameSite=Lax` (+ `Secure` bei TLS-Betrieb). **(4)** Marktplatz prüft das **Gültigkeitsfenster** des Vertrauensankers auch für CRL-/Anker-Dokumente (kein abgelaufener Anker signiert mehr). **(5)** Backup-Wiederherstellung prüft das Datei-Archiv vor dem Entpacken auf Pfad-Traversal (Zip-Slip-Schutz). Bewusst belassen: `/metrics` & Health-Detail für jede angemeldete Identität (Monitoring). **Marktstandort (Kurzfassung):** Sicherheits-/Architektur-/Eventing-Kern teils enterprise-grade; Gesamtreife produktionsreif für Single-Org/On-Prem-SMB; offene Wettbewerbslücken: Mandantenfähigkeit, UI-/Low-Code-Builder, feingranulares RBAC, Such-Integration, Barrierefreiheit, Ökosystem. Verifiziert: volle Suite **206 grün** + Runtime-Smoke. |
| 6.72 | 10.06.2026 | **Restliche Peer-Review-Punkte umgesetzt** (außer zwei bewusst offenen): **(1)** **Anmeldeschutz pro IP vor der Authentifizierung** — eine IP mit zu vielen Fehlversuchen über **beliebige** Konten wird mit `429` abgewiesen, **bevor** ein Passwort geprüft/gehasht wird (gegen Password-Spraying und CPU-Last); die feinkörnige Sperre pro Benutzer bleibt unverändert. **(3)** **Outbox-Idempotenz**: abgeleitete Aktionen (Automation/Workflow) und ihr „erledigt"-Vermerk laufen in **einer** Transaktion → ein Worker-Absturz dazwischen führt nicht mehr zu doppelten Benachrichtigungen/Folge-Ereignissen. **(4/5)** **Semantischer Index** formatiert Vektoren jetzt sprach-/locale-unabhängig und mit höherer Präzision (robust gegen `LC_NUMERIC`). **(6)** **Modul-API-Routen** mit doppeltem Pfad-Parameter werden sauber ignoriert (+ einmalige Log-Warnung) statt einen Regex-Fehler pro Anfrage zu erzeugen. **(7)** **Egress** löst nun IPv4 **und** IPv6 auf, prüft beide und pinnt alle geprüften Adressen → kein Ausweichen über eine ungeprüfte IPv6-Adresse bei Dual-Stack-Hosts. Nicht umgesetzt (auf Wunsch): Rate-Limit-Proxy-Modell und providerübergreifendes SSO-Linking. Nebenbefund behoben: das Worker-Container-Image (ohne `gd` seit P13, daher im Crash-Loop) neu gebaut — der Event-Worker läuft wieder. Verifiziert: 5 neue Tests; volle Suite **201→206 grün** + Runtime-Smoke. |
