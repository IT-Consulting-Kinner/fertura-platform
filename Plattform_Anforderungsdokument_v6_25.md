...
(Erkennen/Stoppen) ist **wrapper-tolerant** (Match auf Host-Skript + Modul-Key in
der Kommandozeile, statt auf ein exaktes Argument). Default ist leer (kein
Prefix). **Bewusst spätere Ausbaustufen** (ehrlich benannt): die OS-Härtung als
**vom Core verwaltete, automatische** Eigenschaft (statt Betreiber-Konfiguration)
inkl. eigenem Container je Modul; sowie **Capability-Tokens je Aufruf** (das
RPC-Token authentifiziert heute den Kanal, nicht den einzelnen Aufruf —
Letzteres ist eine Protokoll-Erweiterung, keine Infrastruktur). **Hinweis
Transaktionsgrenze:** Ein
